...
export const addNewProject = (data) => async (dispatch) => {
  ...
  try {
    const response = await axios.post(
      "https://my-protfolio-ghj2.onrender.com/api/v1/projects/add",
      data,
      {
        withCredentials: true,
...
export const deleteProject = (id) => async (dispatch) => {
  ...
  try {
    const response = await axios.delete(
      `https://my-protfolio-ghj2.onrender.com/api/v1/projects/delete/${id}`,
      {
        withCredentials: true,
      }
    );
...
export const updateProject = (id, newData) => async (dispatch) => {
  ...
  try {
    const response = await axios.put(
      `https://my-protfolio-ghj2.onrender.com/api/v1/projects/update/${id}`,
      newData,
      {
        withCredentials: true,
        headers: { "Content-Type": "multipart/form-data" },
    ...
    dispatch(projectSlice.actions.updateProjectSuccess(response.data.y));
    dispatch(projectSlice.actions.clearAllErrors());
  } catch (error) {
    dispatch(
